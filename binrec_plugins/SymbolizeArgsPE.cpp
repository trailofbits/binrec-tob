...
namespace s2e {
    namespace plugins {
        ...
        {
            m_symbolizeArgv0 =
                s2e()->getConfig()->getBool(getConfigKey() + ".symbolizeArgv0", true);
        ...
        template <class T> static T readMemVal(S2EExecutionState *state, uint64_t addr)
        {
            T val;
        ...
        void SymbolizeArgsPE::slotLibCall(
            ...
            PELibCallMonitor::ReturnSignal &onReturn)
        {
            if (lib.Name == "msvcrt.dll" && routine == "__getmainargs") {
                ...
                plgState->argvAddr =
                    readMemVal<uint32_t>(state, state->getSp() + 2 * sizeof(uint32_t));
        ...
        void SymbolizeArgsPE::makeSymbolic(
            S2EExecutionState *state,
            uint32_t addr,
            size_t size,
            ...
            bool makeConcolic)
        {
            ...
            s2e()->getInfoStream(state)
                << "inserting symbolic data at " << hexval(addr) << " of size " << size
                << " with label \"" << label << "\"\n";
