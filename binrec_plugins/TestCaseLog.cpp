...
S2E_DEFINE_PLUGIN(TestCaseLog,
                  "Log concrete inputs for testcases",
                  "TestCaseLog");

void TestCaseLog::initialize()
{
    std::string logFileName = s2e()->getConfig()->getString(
            getConfigKey() + ".logFile", "testcases.txt");
    m_logFile.open(s2e()->getOutputFilename(logFileName).c_str());
    assert(m_logFile);

    m_onlyPrintable = s2e()->getConfig()->getBool(
            getConfigKey() + ".onlyPrintable", true);
...
void TestCaseLog::slotTestCaseGeneration(S2EExecutionState *state, const std::string &message)
{
