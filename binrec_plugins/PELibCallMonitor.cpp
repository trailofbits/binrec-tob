...
namespace s2e {
    namespace plugins {
        ...
        void PELibCallMonitor::initialize()
        {
            ...
            modEx->onModuleLoad.connect(sigc::mem_fun(*this, &PELibCallMonitor::slotModuleLoad));
        ...
        void
        PELibCallMonitor::slotModuleLoad(S2EExecutionState *state, const ModuleDescriptor &module)
        {
            if (m_mainModuleLoaded && module.Pid == m_moduleDesc.Pid) {
                s2e()->getDebugStream()
                    << "[PELibCallMonitor] module '" << module.Name << "' loaded\n";
                ...
                if (m_mainModuleLoaded) {
                    if (reloadImports(state))
                        s2e()->getDebugStream()
                            << "[PELibCallMonitor] successfully reloaded imports\n";
                    else
        ...
        void PELibCallMonitor::slotMainModuleLoad(
        ...
        void PELibCallMonitor::slotModuleLeave(S2EExecutionState *state, uint64_t pc)
        {
            ...
            if (it != m_symbols.end()) {
                ...
                onLibCall.emit(
                    ...
                    *sym.first,
                    sym.second,
                    m_selector->getPrevLocalPc(state),
                    plgState->onLibReturn);
            } else {
                plgState->callPc = 0;
            ...
            foreach2(it, imports.begin(), imports.end())
            {
                ...
                foreach2(import, functions.begin(), functions.end())
                {
                    ...
                    if (symit == m_symbols.end()) {
                        m_symbols.insert(
                            std::make_pair(address, std::make_pair(library, functionName)));
