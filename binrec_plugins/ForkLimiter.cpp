...
namespace s2e {
    namespace plugins {
        ...
        S2E_DEFINE_PLUGIN(
            ForkLimiter,
            "Limits how many times each instruction in a module can fork, limits max running time of state",
            "ForkLimiter",
            "ModuleSelector");
        ...
        void ForkLimiter::initialize()
        {
            ...
            s2e()->getCorePlugin()->onTranslateBlockStart.connect(
                sigc::mem_fun(*this, &ForkLimiter::onSlotTranslateBlockStart));
            ...
            selector->onModuleLoad.connect(sigc::mem_fun(*this, &ForkLimiter::slotModuleLoad));
        ...
        void ForkLimiter::slotModuleLoad(S2EExecutionState *state, const ModuleDescriptor &module)
        {
        ...
        void
        ForkLimiter::onStateSwitch(S2EExecutionState *currentState, S2EExecutionState *nextState)
        {
            ...
            if (currentState->getID() != 0 && plgState->timerTicks > m_timeLimit) {
                ...
                // s2e()->getExecutor()->terminateStateEarly(*currentState, "maxRunningTime is
                // reached");
            }
            ...
            // s2e()->getDebugStream() << "StateID: " << currentState->getID() << " runningTime: "
            // << plgState->timerTicks << "\n";
        }
            ...
            uint64_t pc)
        {
            ...
            if (plgState->terminate) {
                ...
                if (plgState->reason == forkLimit) {
                    signal->connect(sigc::mem_fun(*this, &ForkLimiter::onTermination2));
                } else if (plgState->reason == timeout) {
                    signal->connect(sigc::mem_fun(*this, &ForkLimiter::onTermination3));
                }
            }
        }
            ...
            const std::vector<klee::ref<klee::Expr>> &newConditions)
        {
            ...
            if (!m_binModule.Contains(state->regs()->getPc())) {
                for (size_t i = 0, e = newStates.size(); i < e; ++i) {
                    ...
                    if (state->getID() == newStates[i]->getID()) {
                        // s2e()->getWarningsStream() << "[[onFork]] newState:" <<
                        // newStates[i]->getID() <<
                        //                       " forkEnabled: " <<
                        //                       newStates[i]->isForkingEnabled() << "\n";
                        continue;
                    }
            ...
            */
            ...
            if (m_forkCount["module"][curPc] > m_forkLimit) {
                // s2e()->getDebugStream() << "StateID: " << state->getID() << " forkLimit
                // reached\n";
                for (size_t i = 0, e = newStates.size(); i < e; ++i) {
                    DECLARE_PLUGINSTATE(ForkLimiterState, newStates[i]);
                    plgState->terminate = true;
