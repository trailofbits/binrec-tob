...
S2E_DEFINE_PLUGIN(BBExport, "Exports LLVM bitcode for an ELF binary", "BBExport", "ModuleSelector");
...
BBExport::~BBExport() { saveLLVMModule(); }
...
void BBExport::slotModuleExecute(S2EExecutionState *state, uint64_t pc) {
    ...
    if (m_currentBlock) {
        ...
        // plt code tricks us to think that we run into tb from previous lifting. Using symbols and 6byte difference for
        // additional info works pretty well.
        if (m_earlyTerminate) {
...
void BBExport::slotModuleLoad(S2EExecutionState *state, const ModuleDescriptor &module) {
    if (!module.Contains(m_address)) {
        s2e()->getWarningsStream() << "[BBExport] module " << module.Name << " (" << hexval(module.LoadBase) << ""
    ...
    m_tbStartConnection =
        s2e()->getCorePlugin()->onTranslateBlockStart.connect(sigc::mem_fun(*this, &BBExport::slotTranslateBlockStart));
    m_tbEndConnection =
        s2e()->getCorePlugin()->onTranslateBlockEnd.connect(sigc::mem_fun(*this, &BBExport::slotTranslateBlockEnd));
}
