...
static RegisterPass<CustomLoopUnroll> X("custom-loop-unroll", "S2E Unroll a loop a given number of times", false,
                                        false);

cl::opt<unsigned> UnrollCount("custom-unroll-count", cl::desc("Number of times to unroll the loop"),
                              cl::value_desc("count"));

cl::opt<std::string> EntryLabel("custom-unroll-entry", cl::desc("Label of entry block for loop to unroll"),
                                cl::value_desc("label"));
...
auto UnrollNTimes(Loop *L, LPPassManager &LPM, unsigned UnrollCount, LoopInfoWrapperPass &LIWP) -> bool {
    LLVM_DEBUG(dbgs() << "Unroll " << UnrollCount << " iterations of loop with entry block " << EntryLabel << "\n");
    ...
    FAILIF(!BI || BI->isConditional(), "  Cander't unroll; loop not terminated by an unconditional branch.\n");
    ...
    for (unsigned It = 1; It != UnrollCount; ++It) {
        ...
        for (auto BB = BlockBegin; BB != BlockEnd; ++BB) {
            ...
            for (succ_iterator SI = succ_begin(*BB), SE = succ_end(*BB); SI != SE; ++SI) {
                if (L->contains(*SI))
                    continue;
                for (BasicBlock::iterator BBI = (*SI)->begin(); auto *phi = dyn_cast<PHINode>(BBI); ++BBI) {
