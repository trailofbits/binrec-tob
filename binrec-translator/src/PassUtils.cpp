...
cl::opt<logging::Level>
    logLevel("loglevel", cl::desc("Logging level"),
             cl::values(clEnumValN(logging::ERROR, "error", "Only show errors"),
...
cl::opt<fallback::Mode> fallbackMode(
    "fallback-mode", cl::desc("How to handle edges that are not recorded in successor lists"),
    cl::values(clEnumValN(fallback::NONE, "none",
                          "Make error block unreachable, allows most optimization"
                          " (assumes a single code path)"),
               clEnumValN(fallback::ERROR1, "error1", "Print prevPC and PC values when execution diverged."),
               ...
               clEnumValN(fallback::UNFALLBACK, "unfallback", "extended + try to return to recovered code")),
    cl::initializer<fallback::Mode>(fallback::NONE));

cl::list<unsigned> breakAt("break-at", cl::desc("Basic block addresses to insert debugging breakpoints at"),
...
auto runlibDir() -> std::string { return s2eRoot() + "/runlib"; }

auto s2eOutFile(const std::string &basename) -> std::string { return basename; }
...
void doInlineAsm(IRBuilder<> &b, StringRef asmText, StringRef constraints, bool hasSideEffects,
                 ArrayRef<Value *> args) {
    ...
    b.CreateCall(
        InlineAsm::get(FunctionType::get(b.getVoidTy(), argTypes, false), asmText, constraints, hasSideEffects), args);
}
