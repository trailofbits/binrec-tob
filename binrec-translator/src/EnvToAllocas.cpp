...
auto isEnvVar(GlobalVariable *glob) -> bool {
    ...
    static set<string> envVarNames = {"PC",    "cc_src", "cc_dst", "cc_op",           "R_EAX",  "R_EBX",
...
auto canBeMoved(GlobalVariable *glob, Function *wrapper) -> bool {
    ...
    for (Use &use : glob->uses()) {
        if (auto *expr = dyn_cast<ConstantExpr>(use)) {
            ...
            if (f != wrapper) {
                DBG("no alloca for " << glob->getName() << ": used outside @wrapper in @" << f->getName());
                return false;
            }
        } else {
...
// NOLINTNEXTLINE
auto EnvToAllocasPass::run(Module &m, ModuleAnalysisManager &mam) -> PreservedAnalyses {
    ...
    for (GlobalVariable &glob : m.globals()) {
        ...
        AllocaInst *alloca = b.CreateAlloca(glob.getType()->getElementType(), nullptr, glob.getName());
