...
auto getSourcePath(Module &m) -> StringRef { return s2eOutFile("binary"); }
...
void copySection(Module &m, section_meta_t &s, std::byte *data, bool readonly) {
    LLVM_DEBUG(dbgs() << "copy section at 0x" << intToHex(s.loadBase) << ": ");
    ...
    s.global = new GlobalVariable(m, initializer->getType(), readonly, GlobalValue::ExternalLinkage, initializer,
                                  s.name, insertBefore);
...
#define NOPERANDS 5
    Metadata *operands[NOPERANDS] = {
        MDString::get(ctx, s.name), ConstantAsMetadata::get(ConstantInt::get(Type::getInt32Ty(ctx), s.loadBase)),
        ConstantAsMetadata::get(ConstantInt::get(Type::getInt32Ty(ctx), s.size)),
        ConstantAsMetadata::get(ConstantInt::get(Type::getInt32Ty(ctx), s.fileOffset)), ValueAsMetadata::get(s.global)};
