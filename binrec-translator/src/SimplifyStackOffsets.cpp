...
static RegisterPass<SimplifyStackOffsets>
    x("simplify-stack-offsets", "S2E Move additions with bitcast stack pointer to getelementptr offset", false, false);
...
auto SimplifyStackOffsets::runOnModule(Module &m) -> bool {
    ...
    for (AddOperator *add : replaceList) {
        ...
        Constant *replacement = ConstantExpr::getPtrToInt(
            ...
            ConstantExpr::getGetElementPtr(nullptr, cast<Constant>(gep->getPointerOperand()), idx, gep->isInBounds()),
            add->getType());
