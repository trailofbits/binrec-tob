...
static RegisterPass<DetectVars> X("detect-vars", "S2E Heuristically split section arrays into global variables", false,
                                  false);
...
static auto getInitializer(GlobalVariable *global, unsigned index, IntegerType *elementType) -> Constant * {
        ...
        if (auto *gep = dyn_cast<GEPOperator>(use)) {
            ...
            for (auto it : sizes) {
                if (overlap(it.first, it.second, index, size)) {
                    errs() << "overlapping globals in " << s.global->getName() << ": (" << it.first << ", " << it.second
                           << ") and (" << index << ", " << size << ")\n";
                    exit(1);
                }
            }
            ...
            auto *g = new GlobalVariable(m, elementType, false, GlobalValue::InternalLinkage, initializer, name);

            DBG("replace " << size << " bytes at index " << index << " of @" << s.global->getName() << " with @"
                           << g->getName());
