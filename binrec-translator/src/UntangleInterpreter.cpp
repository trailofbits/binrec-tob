...
static RegisterPass<UntangleInterpreter>
    X("untangle-interpreter", "S2E Untangle interpreter loop for each distinct virtual program counter value", false,
      false);

cl::opt<std::string> InterpreterEntry("interpreter-entry", cl::desc("Label of entry block of interpreter loop"),
                                      cl::value_desc("label"));

cl::opt<std::string> VpcGlobal("vpc-global", cl::desc("Name of virtual program counter global"),
                               cl::value_desc("global"));

cl::list<std::string> VpcTraceFiles("vpc-trace", cl::desc("Trace files of instrumented interpreter runs"),
                                    cl::value_desc("file"));
...
static auto readTraceFiles(unsigned &entry, std::set<unsigned> &exits, std::set<std::pair<unsigned, unsigned>> &edges)
    -> bool {
