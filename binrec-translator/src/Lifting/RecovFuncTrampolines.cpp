...
struct OrigRecovFuncPair {
    ...
    StoreInst *recovCallTargetAddress = nullptr; // instr when store origCallTargetAddress to "PC"
    StoreInst *recovRetAddress = nullptr;        // instr when store origRetAddress to "PC",
                                                 // go back to trampoline after this

    OrigRecovFuncPair(unsigned octa, unsigned ora) : origCallTargetAddress(octa), origRetAddress(ora) {}
    ...
    //                                      if (ConstantInt *ci1 = dyn_cast<ConstantInt>(loadInst)) {
...
auto readInFuncs(std::vector<OrigRecovFuncPair> *out, const std::string filename) -> bool {
    std::ifstream inFile;
...
auto writeOutFuncsToOverwrite(std::vector<OrigRecovFuncPair> *out, const std::string filename) -> bool {
...
//     for (auto iter = containingF->getBasicBlockList().begin(); iter != containingF->getBasicBlockList().end();
//     iter++) {
...
auto makeEnterTramp(Module &m, OrigRecovFuncPair *recovFunc) -> BasicBlock * {
    Function *wrapper = m.getFunction("Func_wrapper");
    assert(wrapper);
    BasicBlock *enterTrampoline = BasicBlock::Create(
        m.getContext(), "enterTrampoline" + std::to_string(recovFunc->origCallTargetAddress), wrapper);
    ...
    std::array<Value *, 9> args = {
        m.getNamedGlobal("R_EAX"), m.getNamedGlobal("R_EBX"), m.getNamedGlobal("R_ECX"),
        m.getNamedGlobal("R_EDX"), m.getNamedGlobal("R_ESI"), m.getNamedGlobal("R_EDI"),
        m.getNamedGlobal("R_ESP"), m.getNamedGlobal("R_EBP"), magicNum,
    };
    doInlineAsm(b,
                "jmp v${:uid}\n\t"
                "movl $8, %eax\n\t"
...
auto makeExitTramp(Module &m) -> BasicBlock * {
    Function *wrapper = m.getFunction("Func_wrapper");
    assert(wrapper);
    ...
    std::array<Value *, 8> args2 = {m.getNamedGlobal("R_EAX"), m.getNamedGlobal("R_EBX"), m.getNamedGlobal("R_ECX"),
                                    m.getNamedGlobal("R_EDX"), m.getNamedGlobal("R_ESI"), m.getNamedGlobal("R_EDI"),
                                    m.getNamedGlobal("R_ESP"), m.getNamedGlobal("R_EBP")};
    doInlineAsm(exitB,
                "movl $6, %esp\n\t"
                "movl $0, %eax\n\t"
                "movl $1, %ebx\n\t"
...
class RecovFuncTrampolinesPass : public PassInfoMixin<RecovFuncTrampolinesPass> {
public:
    ...
    auto run(Module &m, ModuleAnalysisManager &) -> PreservedAnalyses {
        ...
        for (OrigRecovFuncPair recovFunc : recovFunctions) {
            ...
            Instruction *thenT = llvm::SplitBlockAndInsertIfThen(onUnfallback, termOfFuncExitBlock, true);
            // llvm 3.2 api
...
void binrec::addRecovFuncTrampolinesPass(ModulePassManager &mpm) { mpm.addPass(RecovFuncTrampolinesPass()); }
