...
def _debug_trace_info(ti : Path, doprint=True):
    with open(ti, 'r') as f:
        ti = json.load(f)
...
def _diff_trace_info(paths: Iterable[Path], show_common=False):
    ...
    # Compare successors
    dict2tuple = lambda x: (x["pc"], x["successor"])
    ...
    #compare entryToTbs
    etbs2tuple = lambda x: (x[0], tuple(sorted(x[1])))
...
def main() -> None:
    ...
    dbg.add_argument("paths", nargs=2, type=Path, help="Diff of two traceInfo.json files")
    dbg.add_argument("--show_common", help="Show entries common to both files", action='store_true')
