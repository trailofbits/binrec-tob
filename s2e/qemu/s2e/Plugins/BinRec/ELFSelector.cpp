...
S2E_DEFINE_PLUGIN(ELFSelector, "Emits execution events only for blocks in an ELF binary", "ModuleSelector",
                  "ModuleExecutionDetector");

void ELFSelector::initialize() {
    m_moduleInitialized = false;

    ModuleExecutionDetector *modEx = (ModuleExecutionDetector *)(s2e()->getPlugin("ModuleExecutionDetector"));
    assert(modEx);
...
void ELFSelector::slotTranslateBlockStart(ExecutionSignal *signal, S2EExecutionState *state, TranslationBlock *tb,
                                          uint64_t pc) {
...
void ELFSelector::slotModuleTranslateBlockStart(ExecutionSignal *signal, S2EExecutionState *state,
...
ELFSelectorState::ELFSelectorState() : pcInModule(false), prevPcInModule(false), inLibCall(true), prevPc(0) {}
