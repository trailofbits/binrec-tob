...
bool S2EExtractAndBuildPass::runOnFunction(Function &F) {
  ...
  for (Function::iterator ibb = F.begin(), ibe = F.end(); ibb != ibe; ++ibb) {
    ...
    for (BasicBlock::iterator iib = ibb->begin(), iie = ibb->end(); iib != iie; ++iib) {
      ...
      if (!storeInst)
        continue;
      ConstantInt *constValue = dyn_cast<ConstantInt>(storeInst->getValueOperand());
      if (storeInst->getPointerOperand()->getName() == "PC") {
        if (!constValue)
          targetMap.erase(storeInst->getParent());
        else
          targetMap[storeInst->getParent()] = constValue->getZExtValue();
      }
      else if (nextBlockAddr != (unsigned long)-1 && constValue && (constValue->getZExtValue() & -2) == (nextBlockAddr & -2)) {
        if (storeInst->getPointerOperand()->getName() == "LR")
          implicitTarget = true;
      }
    }
  }
...
BasicBlock* S2EExtractAndBuildPass::getBasicBlockByAddr(uint64_t addr) {
    return getBasicBlock("BB_" + intToString(addr));
}

void S2EExtractAndBuildPass::initialize(Function &F) {
  M = new Module("S2E-Module", F.getContext());
  genericFunctionType = FunctionType::get(Type::getInt1Ty(F.getContext()), false);
  initialized = true;
}
...
bool S2EExtractAndBuildPass::searchNextBasicBlockAddr(Function &F) {
  ...
  for (Function::iterator ibb = F.begin(), ibe = F.end(); ibb != ibe; ++ibb) {
    for (BasicBlock::iterator iib = ibb->begin(), iie = ibb->end(); iib != iie; ++iib) {
      ...
      if (storeInst) {
        ConstantInt *constValue = dyn_cast<ConstantInt>(storeInst->getValueOperand());
        if (constValue && constValue->getZExtValue() == nextBlockAddr)
          if (storeInst->getPointerOperand()->getName() == "LR")
          {
        	  implicitTarget = true;
        	  return true;
...
void S2EExtractAndBuildPass::createIndirectCallEdge(BasicBlock *block) {
  ...
  Type* genericFunctionPtrTy = PointerType::get(genericFunctionType, 0);
