...
static RegisterPass<PEMainPass> X("pe-main",
        "S2E Add main function that allocates stack and calls wrapper (PE)",
        false, false);

bool PEMainPass::runOnModule(Module &m) {
    m.setTargetTriple(TARGET_TRIPLE);

    LLVMContext &ctx = m.getContext();
    Function *main = cast<Function>(m.getOrInsertFunction(
                MAIN_FUNCTION_NAME, Type::getVoidTy(ctx), NULL));
