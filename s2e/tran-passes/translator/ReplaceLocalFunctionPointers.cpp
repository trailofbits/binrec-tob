...
static RegisterPass<ReplaceLocalFunctionPointers> X("replace-local-function-pointers",
...
bool ReplaceLocalFunctionPointers::doInitialization(Module &m) {
...
bool ReplaceLocalFunctionPointers::runOnFunction(Function &f) {
    //DBG("called rlfp \n");
        ...
        //for (auto &inst : bb){
            //if (!isa<SwitchInst>(&inst)){
                //for (auto &use : inst.operands()){
                    //ifcast(ConstantInt, ciop, use ){
                        //lookup = allOriginalFunctions.find(ciop->getZExtValue());
                        //if (lookup  != allOriginalFunctions.end()){
                        ////if (std::map<>::iterator lookup = allOriginalFunctions.find(ciop->getZExtValue()) != allOriginalFunctions.end()){
    ...
    DBG("---------------ALL STORES----------------");
    for (auto &bb : f){
        foreach(instruction , bb){ //code assumes instructions visited in order
        ...
        for (auto &use : call->operands()){
            ifcast(ConstantInt, ciop, use ){
                ...
                //if (std::map<>::iterator lookup = allOriginalFunctions.find(ciop->getZExtValue()) != allOriginalFunctions.end()){
                    ...
                    Value *sym = b.CreateBitOrPointerCast(BlockAddress::get(lookup->second), IntegerType::getInt32Ty(ctx));
                    use = sym;
