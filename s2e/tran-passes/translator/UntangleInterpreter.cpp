...
static GlobalVariable *findVpcGlobal(Module &m) {
...
static void cloneLoopBody(Loop *L, unsigned vpcValue, ValueToValueMapTy &VMap) {
    ...
    for (BasicBlock *bb : newBlocks)
        for (Instruction &inst : *bb)
            RemapInstruction(&inst, VMap, RF_NoModuleLevelChanges | RF_IgnoreMissingEntries);
}

const HeaderLatchPair &UntangleInterpreter::getOrCreatePair(const unsigned vpcValue) {
    auto it = vpcMap.find(vpcValue);

    if (it == vpcMap.end()) {
        ValueToValueMapTy VMap;
        cloneLoopBody(interpreter, vpcValue, VMap);
        BasicBlock *newHeader = cast<BasicBlock>(VMap[interpreter->getHeader()]);
        BasicBlock *newLatch = cast<BasicBlock>(VMap[interpreter->getLoopLatch()]);

        // Replace backedge with switch statement
        BranchInst *br = dyn_cast<BranchInst>(newLatch->getTerminator());
        assert(br);
        ...
        if (!errBlock) {
            errBlock = BasicBlock::Create(newLatch->getContext(), "error",
                    newLatch->getParent());
...
bool UntangleInterpreter::runOnLoop(Loop *L, LPPassManager &LPM) {
    ...
    IntegerType *vpcTy = cast<IntegerType>(vpc->getType()->getElementType());
    ...
    for (auto &iedge : edges) {
        ...
        SwitchInst *sw = cast<SwitchInst>(srcPair.second->getTerminator());
