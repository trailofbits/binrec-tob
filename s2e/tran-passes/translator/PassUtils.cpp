...
cl::opt<fallback::Mode> fallbackMode("fallback-mode",
        ...
        cl::values(
            ...
            clEnumValN(fallback::UNFALLBACK, "unfallback",
...
std::unique_ptr<Module> loadBitcodeFile(StringRef path) {
    SMDiagnostic err;
    std::unique_ptr<Module> from = parseIRFile(path, err, getGlobalContext());
