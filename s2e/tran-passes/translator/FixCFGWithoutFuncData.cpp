...
static RegisterPass<FixCFGWithoutFuncData> X("fix-cfg-without-func-data",
...
bool FixCFGWithoutFuncData::runOnModule(Module &m) {
    ...
    for(BasicBlock *B : BBSet){
        ...
        DBG("libCallBB: " << B->getName() << " predBB: " << (*pred_begin(B))->getName() << " nextBB: " << nextBB->getName() << " addCase: " << addCase);
        if(addCase){
