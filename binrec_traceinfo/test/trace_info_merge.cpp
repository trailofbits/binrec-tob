...
TEST(trace_info_merge, stack_frame_sizes) {
    ...
    EXPECT_THAT(base.stackFrameSizes, UnorderedElementsAre(
        Pair("first", 3),
        Pair("second", 2),
        Pair("third", 1)));
}
...
TEST(trace_info_merge, stack_differences) {
    ...
    EXPECT_THAT(base.stackDifference, UnorderedElementsAre(
        Pair("first", 100),
        Pair("second", 200),
        Pair("third", 300)
    ));
...
TEST(trace_info_merge, successors) {
    ...
    EXPECT_THAT(base.successors, UnorderedElementsAre(
...
TEST(trace_info_merge, function_log_entry_to_caller) {
    ...
    EXPECT_THAT(base.functionLog.entryToCaller, UnorderedElementsAre(
        Pair(1, 2),
        Pair(3, 4),
        Pair(5, 6),
        Pair(7, 8)
...
TEST(trace_info_merge, function_log_entry_to_return) {
    ...
    EXPECT_THAT(base.functionLog.entryToReturn, UnorderedElementsAre(
        Pair(1, 2),
        Pair(3, 4),
        Pair(5, 6),
        Pair(7, 8)
...
TEST(trace_info_merge, function_log_caller_to_followup) {
    ...
    EXPECT_THAT(base.functionLog.callerToFollowUp, UnorderedElementsAre(
        Pair(1, 2),
        Pair(3, 4),
        Pair(5, 6),
        Pair(7, 8)
...
TEST(trace_info_merge, function_log_entry_to_tbs) {
    ...
    EXPECT_THAT(base.functionLog.entryToTbs, UnorderedElementsAre(
