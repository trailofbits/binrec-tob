...
CallStack::LiveStackFrame::LiveStackFrame(
    StackFrame *frame,
...
auto CallStack::LiveStackFrame::inCall() const -> bool
{
    return inCallAt != InstructionAddress{};
}

auto CallStack::LiveStackFrame::range() const -> StackFrameRange
{
    return {base, top};
}
...
{
    std::cout << "Record call at " << callSite << " to " << target << " (stack = " << top << ")"
              << std::endl;
    if (activeFrames.lower_bound(top) != activeFrames.end()) {
        std::cout << "Active: " << activeFrames.rbegin()->second->base.value
                  << ", top: " << top.value << std::endl;
        assert(
            activeFrames.lower_bound(top) == activeFrames.end() &&
    ...
    auto liveFrame =
        std::make_unique<LiveStackFrame>(frame->second.get(), std::move(topFrame), top);
...
void CallStack::recordStackPointerUpdate(InstructionAddress at, StackAddress value)
{
    ...
    std::cout << "Update stack pointer at " << at << " to " << value << " ("
              << value - topFrame->top << ")"
              << "\n";
