...
namespace s2e {
    namespace plugins {
        ...
        void DebugLog::initialize()
        {
            ...
            selector->onModuleLoad.connect(sigc::mem_fun(*this, &DebugLog::slotModuleLoad));
            selector->onModuleExecute.connect(sigc::mem_fun(*this, &DebugLog::slotModuleExecute));
            selector->onModuleLeave.connect(sigc::mem_fun(*this, &DebugLog::slotModuleLeave));
            selector->onModuleReturn.connect(sigc::mem_fun(*this, &DebugLog::slotModuleReturn));

            PELibCallMonitor *mon = (PELibCallMonitor *)(s2e()->getPlugin("PELibCallMonitor"));
            if (mon)
                mon->onLibCall.connect(sigc::mem_fun(*this, &DebugLog::slotLibCall));
        }

        void DebugLog::slotModuleLoad(S2EExecutionState *state, const ModuleDescriptor &module)
        {
