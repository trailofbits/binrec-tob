...
header "insert calls between recovered functions"
queue insert-calls
run-queue
...
mymake -s ${outfile%.bc}.ll
cp ${outfile%.bc}.ll beforestacksym.ll

header "stack symbolization"
#queue stack-symbolizer
run-queue

header "mem2reg for stack symbolizer"
#queue mem2reg

run-queue

mymake -s ${outfile%.bc}.ll
cp ${outfile%.bc}.ll afterstacksym.ll
...
header "dead code elimination to check if redundant loads are removed"
queue globaldce
run-queue
queue dce
run-queue

# header "recover functions"
# queue recover-functions
