...
# Create a link to the binary having the name binary
# to make s2eout_makefile happy
if [ ! -f "binary" ]; then
  binary=$(basename $(cd .. && pwd))
  ln -s ../$binary binary
fi
...
"$S2EDIR/../build/bin/binrec_lift" --loglevel=debug --lift -o lifted linked.bc --clean-names
