...
StackFrame::StackFrame(FunctionAddress functionAddress, InstructionAddress callerAddress)
    : functionAddress{functionAddress}, callerAddress{callerAddress} {}

StackFrame::StackFrame(const Data &data)
    : functionAddress{data.id.functionAddress}, callerAddress{data.id.callerAddress},
      spUpdates{data.spUpdates.begin(), data.spUpdates.end()}, fpUpdates{data.fpUpdates.begin(), data.fpUpdates.end()} {
}

auto StackFrame::id() const -> StackFrame::Id { return {functionAddress, callerAddress}; }
...
void StackFrame::recordStackPointerUpdate(InstructionAddress at, Bytes value) { spUpdates.emplace(at, value); }

void StackFrame::recordFramePointerUpdate(InstructionAddress at, Bytes value) { fpUpdates.emplace(at, value); }
