...
void to_json(json &j, const MemoryAccess &ma) {
    j = json{{"pc", ma.pc},     {"offset", ma.offset},     {"isWrite", ma.isWrite}, {"isLocalAccess", ma.isLocalAccess},
             {"size", ma.size}, {"isDirect", ma.isDirect}, {"fnBase", ma.fnBase}};
}
...
void to_json(json &j, const TraceInfo &ti) {
    j = json{{"stackSizes", ti.stackFrameSizes},
...
void TraceInfo::add(const TraceInfo &ti) {
    ...
    std::copy(ti.memoryAccesses.begin(), ti.memoryAccesses.end(), std::back_inserter(memoryAccesses));
    std::copy(ti.successors.begin(), ti.successors.end(), std::inserter(successors, successors.begin()));

    if (functionLog.entries.empty()) {
        std::copy(ti.functionLog.entries.begin(), ti.functionLog.entries.end(),
                  std::back_inserter(functionLog.entries));
    }
    std::copy(ti.functionLog.entryToCaller.begin(), ti.functionLog.entryToCaller.end(),
              std::inserter(functionLog.entryToCaller, functionLog.entryToCaller.begin()));
    std::copy(ti.functionLog.entryToReturn.begin(), ti.functionLog.entryToReturn.end(),
              std::inserter(functionLog.entryToReturn, functionLog.entryToReturn.begin()));
    std::copy(ti.functionLog.callerToFollowUp.begin(), ti.functionLog.callerToFollowUp.end(),
