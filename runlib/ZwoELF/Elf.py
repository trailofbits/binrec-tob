...
class Section(object):
    ...
    def dump_hdr(self):
        print '  Name              Type            Addr     Off    Size   ES Flg Lk Inf Al'

    def dump(self):
        h = self.elfN_shdr
        print '  %-17s %-15s %08x %06x %06x %02x %3s %2d %3d %2d' % \
                (self.sectionName, SH_type.reverse_lookup[h.sh_type],
                        h.sh_addr, h.sh_offset, h.sh_size, h.sh_entsize,
                        SH_flags.stringify(h.sh_flags), h.sh_link, h.sh_info,
                        h.sh_addralign)
...
class Segment(object):
    ...
    def dump(self):
        h = self.elfN_Phdr
        print '  %-14s 0x%08x %08x 0x%08x 0x%05x 0x%05x %3s 0x%x' % \
                (P_type.reverse_lookup[h.p_type], h.p_offset, h.p_vaddr,
                        h.p_paddr, h.p_filesz, h.p_memsz,
                        P_flags.stringify(h.p_flags), h.p_align)
...
class SH_flags(object):
    ...
    def stringify(cls, fl):
        ...
        if fl & cls.SHF_WRITE:
            s += 'W'
        if fl & cls.SHF_EXECINSTR:
            s += 'X'
        if fl & cls.SHF_MASKPROC:
            s += 'MS'
...
class P_flags(object):
    ...
    @classmethod
    def stringify(cls, fl):
        return '%c%c%c' % ('R' if fl & cls.PF_R else ' ',
                           'W' if fl & cls.PF_W else ' ',
                           'E' if fl & cls.PF_X else ' ')
