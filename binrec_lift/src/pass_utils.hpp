#ifndef BINREC_PASS_UTILS_HPP
#define BINREC_PASS_UTILS_HPP
...
static inline auto hexToInt(llvm::StringRef hexStr) -> uint32_t
{
    return std::strtoul(hexStr.data(), nullptr, 16);
}
...
static inline auto getBlockAddress(llvm::Function *f) -> unsigned
{
    return hexToInt(f->getName().substr(5));
}

static inline auto getBlockAddress(llvm::BasicBlock *bb) -> unsigned
{
