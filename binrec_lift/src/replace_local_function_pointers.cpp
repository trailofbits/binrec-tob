...
static RegisterPass<ReplaceLocalFunctionPointers> X(
    "replace-local-function-pointers",
    "Determine original address of local functions, determine corresponding lifted local functions, replace usages of original with lifted version",
    false,
    false);
    ...
    for (auto &call : stores) {
        ...
        for (auto &use : call->operands()) {
            if (auto *ciop = dyn_cast<ConstantInt>(use)) {
                lookup = allOriginalFunctions.find(ciop->getZExtValue());
                if (lookup != allOriginalFunctions.end()) {
                    // if (std::map<>::iterator lookup =
                    // allOriginalFunctions.find(ciop->getZExtValue()) !=
                    // allOriginalFunctions.end()){
                    call->print(llvm::errs());
                    ...
                    Value *sym = b.CreateBitOrPointerCast(
                        BlockAddress::get(lookup->second),
